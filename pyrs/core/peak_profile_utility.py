...
class BackgroundFunction(Enum):
    ...
    def native_parameters(self):
        ...
        NATIVE_BACKGROUND_PARAMETERS = {"Linear": ["A0", "A1"], "Quadratic": ["A0", "A1", "A2"]}
...
class Gaussian(PeakParametersConverter):
    ...
    def calculate_effective_parameters(self, param_value_array, param_error_array):
        ...
        if param_value_array.dtype != param_error_array.dtype:
            raise RuntimeError(
                "dtype of values and errors do not match: {} and {}".format(
                    param_value_array.dtype, param_error_array.dtype
                )
            )
        if param_value_array.size != param_error_array.size:
            raise RuntimeError(
                "size of values and errors do not match: {} and {}".format(
                    param_value_array.size, param_error_array.size
                )
            )
        ...
        intensity_array = self.cal_intensity(param_value_array["Height"], param_value_array["Sigma"])
        ...
        eff_value_array["Mixing"] = 1.0  # no mixing for Gaussian
        ...
        intensity_error_array = self.cal_intensity_error(
            intensity_array,
        ...
        eff_error_array["Mixing"] = 0.0  # no uncertainty in mixing for Gaussian
    ...
    def cal_intensity(height, sigma):
        ...
        """
        intensity = np.sqrt(2.0 * np.pi) * height * sigma
    ...
    def cal_fwhm_error(sigma_error):
        ...
        """
        fwhm_error = 2.0 * np.sqrt(2.0 * np.log(2.0)) * sigma_error
...
class PseudoVoigt(PeakParametersConverter):
    ...
    def calculate_effective_parameters(self, param_value_array, param_error_array):
        ...
        heights = self.cal_height(
            intensity=param_value_array["Intensity"],
        ...
        heights_error = self.cal_height_error(
            param_value_array["Intensity"],
    ...
    def cal_height(intensity, fwhm, mixing):
        ...
        Float/ndarray
            peak height
        """
        height = 2.0 * intensity * (1 + (np.sqrt(np.pi * np.log(2)) - 1) * mixing) / (np.pi * fwhm)
    ...
    def cal_height_error(intensity, intensity_error, fwhm, fwhm_error, mixing, mixing_error):
        ...
        # sum
        s_h2 = (
            (part_h_part_i * intensity_error) ** 2
            + (part_h_part_gamma * fwhm_error) ** 2
...
def calculate_profile(peak_type, background_type, vec_x, param_value_dict, peak_range):
    ...
    if peak_type == str(PeakShape.GAUSSIAN):
        # Gaussian: ['Height', 'PeakCentre', 'Sigma']
        vec_intensity[left_x_index:right_x_index] = gaussian(
            vec_x[left_x_index:right_x_index],
        ...
        vec_intensity[left_x_index:right_x_index] = pseudo_voigt(
            vec_x[left_x_index:right_x_index],
    ...
    if str(background_type) == str(BackgroundFunction.LINEAR):
        # Linear background
        bkgd_i = quadratic_background(
            vec_x[left_x_index:right_x_index], b0=param_value_dict["A0"], b1=param_value_dict["A1"], b2=0.0, b3=0.0
        )
...
def pseudo_voigt(x, intensity, fwhm, mixing, x0):
    ...
    part_gauss = 1.0 / (sigma * np.sqrt(2.0 * np.pi)) * np.exp(-((x - x0) ** 2) / (2 * sigma**2))

    # Calculate normalized Lorentzian
    part_lorenz = 1.0 / np.pi * (fwhm / 2.0) / ((x - x0) ** 2 + (fwhm / 2.0) ** 2)
