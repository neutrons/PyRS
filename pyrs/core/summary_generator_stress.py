        ...
        for direction in SummaryGeneratorStress.directions:
            ...
            if not strain.filenames:
                if direction == "11" or direction == "22":
                    raise RuntimeError(
                        "StrainField filenames in direction "
                        + str(direction)
                        + " can't be empty for Stress CSV output "
                        + self._filename
                    )
                elif direction == "33":
            ...
            def _write_field_3d(row: int, field: str):
                ...
                for direction in SummaryGeneratorStress.directions:
                    if field == "Strain":
                        ...
                        entries += self._write_number(strain_value, decimals) + self._write_number(
                            strain_error, decimals
                        )

                    elif field == "Stress":
                        stress_value = self._stress_field[direction][0][row]
                        stress_error = self._stress_field[direction][1][row]
                        entries += self._write_number(stress_value, decimals) + self._write_number(
                            stress_error, decimals
                        )

                    else:
                        # by pass calculated direction as there are no measured quantities
                        if (
                            self._stress_facade.stress_type == "in-plane-strain"
                            or self._stress_facade.stress_type == "in-plane-stress"
                        ) and direction == "33":
                            entries += ", , "
                            continue
                        ...
                        entries += self._write_number(value, decimals) + self._write_number(error, decimals)
            ...
            for row, coordinate in enumerate(self._stress.coordinates):
                ...
                for direction in SummaryGeneratorStress.directions:
                    ...
                    if row >= len(peak_collection.get_d_reference()[0]):
                        ...
                        line += self._write_number(d0_value, decimals) + self._write_number(d0_error, decimals)
                        break
            ...
            for field_3dir in SummaryGeneratorStress.fields_3dir:
                for direction in SummaryGeneratorStress.directions:
                    ...
                    if field_3dir == "Stress":
                        stress_fields[direction] = (
                            self._stress_facade.stress.values,
                        ...
                        for run in runs:
                            ...
                            if field_3dir == "Strain":
                                strain_fields[direction][run] = (
                                    self._stress_facade.strain.values,
                                ...
                                peak_data[direction][run][field_3dir] = (field_data.values, field_data.errors)
            # write for each row of the CSV body, first coordinates, d0 and
            # then fields in SummaryGeneratorStress.fields_3dir value, error per dimension
            for row, coordinate in enumerate(self._stress.coordinates):
                line = ""
                ...
                for field_3dir in SummaryGeneratorStress.fields_3dir:
                    ...
                    for direction in SummaryGeneratorStress.directions:
                        ...
                        if field_3dir == "Stress":
                            ...
                            line += self._write_number(stress_value, decimals) + self._write_number(
                                stress_error, decimals
                            )
                        else:
                            runs = self._stress_facade.runs(direction)
                            for run in runs:
                                ...
                                if field_3dir == "Strain":
                                    ...
                                    line += self._write_number(strain_value, decimals) + self._write_number(
                                        strain_error, decimals
                                    )
                                else:
                                    ...
                                    line += self._write_number(peak_parameter_value, decimals) + self._write_number(
                                        peak_parameter_error, decimals
                                    )
    ...
    def _recalc_peak_collections_data(self):
        for field in SummaryGeneratorStress.fields_3dir:
            for direction in SummaryGeneratorStress.directions:
                ...
                if field == "Stress":
                    self._stress_field[direction] = (
                        self._stress_facade.stress.values,
                        self._stress_facade.stress.errors,
                    )

                elif field == "Strain":
                    self._strain_field[direction] = (
                        self._stress_facade.strain.values,
                        self._stress_facade.strain.errors,
                    )
                    ...
                    self._peak_colllections_data[field][direction] = (field_data.values, field_data.errors)
