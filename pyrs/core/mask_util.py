...
def load_mantid_mask(pixel_number, mantid_mask_xml, is_mask):
    ...
    for masked_det_pair in masked_det_pair_list:
        ...
        if end_detid >= pixel_number:
            raise RuntimeError(
                "Detector ID {} is out of range of given detector size {}".format(end_detid, pixel_number)
            )
        # mask or ROI
        if is_mask:
            masking_array[start_detid : end_detid + 1] = 0.0
        else:
            masking_array[start_detid : end_detid + 1] = 1.0
        # stat
        masked_specs += end_detid - start_detid + 1
    # END-FOR

    print("[DB...CHECK] Masked spectra = {}, Sum of masking array = {}".format(masked_specs, sum(masking_array)))
...
def save_mantid_mask(mask_vec, h5_name: str, two_theta: float, note: Optional[str] = None) -> None:
    ...
    if two_theta is not None:
        two_theta = to_float("2-theta", two_theta, -360.0, 360)
    if note is not None:
