...
class MantidPeakFitEngine(PeakFitEngine):
    ...
    def __init__(self, hidraworkspace, peak_function_name, background_function_name, out_of_plane_angle, wavelength):
        super(MantidPeakFitEngine, self).__init__(
            hidraworkspace,
            peak_function_name,
    ...
    def fit_peaks(self, peak_tag, x_min, x_max):
        ...
        peak_collection = self.__tables_to_peak_collection(
            peak_tag, fit_return.OutputPeakParametersWorkspace, fit_return.OutputParameterFitErrorsWorkspace
        )
        ...
        return FitResult(
            peakcollections=(peak_collection,), fitted=fit_return.FittedPeaksWorkspace, difference=difference
        )
    ...
    def __tables_to_peak_collection(self, peak_tag, table_params, table_errors):
        ...
        peak_object = PeakCollection(
            peak_tag=peak_tag,
            peak_profile=self._peak_function,
        ...
        peak_object.set_peak_fitting_values(
            self._subruns, peak_params_value_array, peak_params_error_array, fit_cost_array
        )
