...
class NTableWidget(QTableWidget):
    ...
    def append_row(self, row_value_list, type_list=None, num_decimal=7):
        ...
        if len(row_value_list) != self.columnCount():
            ret_msg = "Input number of values (%d) is different from column number (%d)." % (
                len(row_value_list),
                self.columnCount(),
            )
    ...
    def export_table_csv(self, csv_file_name):
        ...
        assert isinstance(csv_file_name, str), (
            "CSV file name {0} to export table must be a string but not a {1}".format(
                csv_file_name, type(csv_file_name)
            )
        )
    ...
    def get_cell_value(self, row_index, col_index, allow_blank=False):
        ...
        if cell_data_type == "checkbox":
            ...
            assert isinstance(item_i_j, QTableWidgetItem), (
                "Cell {0} {1} must be of type QTableWidgetItem but not a {2}".format(
