...
class EventHandler:
    ...
    def load_run_number_plot(self):
        ...
        try:
            project_dir = [
                get_input_project_file(run, preferredType=self.parent.ui.comboBox_reduction.currentText())
                for run in runs
            ]
        except (FileNotFoundError, RuntimeError, ValueError) as run_err:
            pop_message(self, f"Failed to find run {self.parent.ui.lineEdit_expNumber.text()}", str(run_err), "error")
            return
    ...
    def browse_load_plot_hdf(self):
        ...
        if hidra_file_name is None:
            ...
            hidra_file_name = browse_file(
                self.parent, "HIDRA Project File", os.getcwd(), file_filter, file_list=True, save_file=False
            )
    ...
    def load_and_plot(self, hidra_file_name):
        try:
            ...
            pop_message(self, "Failed to load {}".format(hidra_file_name), str(run_err), "error")

        except KeyError as key_err:
            pop_message(self, "Failed to load {}".format(hidra_file_name), str(key_err), "error")

        self.parent.current_root_statusbar_message = "Working with: {} \t\t\t\t Project Name: {}".format(
            self.parent._curr_file_name, self.parent._project_name
        )
    ...
    def __retrieving_json_file_name(self, save_file=True):
        ...
        json_file_name = browse_file(
            self.parent, "Peak Range File", os.getcwd(), file_filter, file_list=False, save_file=save_file
        )
    ...
    def save_peak_range(self):
        ...
        for _index, peak_range in enumerate(list_peak_ranges):
            dict[_index] = {"peak_range": peak_range, "peak_label": list_peak_labels[_index], "d0": list_peak_d0}
