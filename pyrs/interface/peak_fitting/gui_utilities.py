...
class GuiUtilities:
    ...
    def enabled_sub_runs_interation_widgets(self, enabled=True):
        ...
        self.enabled_list_widgets(list_widgets=list_widgets, enabled=enabled)
    ...
    def enabled_export_csv_widgets(self, enabled=True):
        list_widgets = [
            self.parent.ui.pushButton_exportCSV,
        ]
        self.enabled_list_widgets(list_widgets=list_widgets, enabled=enabled)

    def enabled_peak_ranges_widgets(self, enabled=True):
        list_widgets = [
            self.parent.ui.groupBox_peak_ranges,
        ]
        self.enabled_list_widgets(list_widgets=list_widgets, enabled=enabled)
    ...
    def enabled_2dplot_widgets(self, enabled=True):
        list_widgets = [
            self.parent.ui.frame_2dplot,
            self.parent.ui.graphicsView_2dPlot_frame,
        ]
        self.enabled_list_widgets(list_widgets=list_widgets, enabled=enabled)

    def enabled_save_files_widget(self, enabled=False):
        list_widgets = [self.parent.ui.actionSave, self.parent.ui.actionSaveAs]
        self.enabled_list_widgets(list_widgets=list_widgets, enabled=enabled)
    ...
    def check_subRuns_display_mode(self):
        ...
        list_ui_individual = [self.parent.ui.horizontalScrollBar_SubRuns, self.parent.ui.label_SubRunsValue]
        list_ui_listruns = [self.parent.ui.label_listSubRunsHelp, self.parent.ui.lineEdit_listSubRuns]

        self.enabled_list_widgets(list_widgets=list_ui_individual, enabled=individual_radio_button_status)
        self.enabled_list_widgets(list_widgets=list_ui_listruns, enabled=(not individual_radio_button_status))
    ...
    def set_1D_2D_axis_comboboxes(self, with_clear=False, fill_raw=False, fill_fit=False):
        ...
        sample_log_names = self.parent.model.get_sample_log_names()

        list_ui = [
            self.parent.ui.comboBox_xaxisNames,
    ...
    def initialize_combobox_peak_label(self):
        ...
        list_ui = [
            self.parent.ui.plot1d_xaxis_peak_label_comboBox,
    ...
    def _update_plots_1D_combobox_items(self, list_axis_to_plot=[]):
        _list_comboboxes = [self.parent.ui.comboBox_xaxisNames, self.parent.ui.comboBox_yaxisNames]
    ...
    def _update_plots_2D_combobox_items(self):
        _list_xy_comboboxes = [self.parent.ui.comboBox_xaxisNames_2dplot, self.parent.ui.comboBox_yaxisNames_2dplot]
    ...
    def make_visible_peak_label_of_1d_widgets(self, visible=True):
        list_ui = [self.parent.ui.plot1d_peak_label, self.parent.ui.plot1d_peak_label_comboBox]
        GuiUtilities.make_visible_ui(list_ui=list_ui, visible=visible)
    ...
    def fill_peak_range_table(self, list_fit_peak_ranges=[], list_fit_peak_labels=[], list_fit_peak_d0=[]):
