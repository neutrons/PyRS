...
class Plot:
    ...
    def plot_diff_and_fitted_data(self, sub_run_number):
        ...
        diff_data_set = self.parent._core.get_diffraction_data(
            session_name=self.parent._project_name, sub_run=sub_run_number, mask=None
        )

        # Plot experimental data
        data_set_label = "Scan {0}".format(sub_run_number)
        self.parent._ui_graphicsView_fitSetup.plot_experiment_data(
            diff_data_set=diff_data_set, data_reference=data_set_label
        )
    ...
    def plot_2d(self, sub_run_list=None):
        ...
        try:
            ...
            if (axis_x_data.size == np.unique(axis_x_data).size) or (axis_x_data.size == np.unique(axis_x_data).size):
                plot_scatter = True
            ...
            self.parent.ui.graphicsView_plot2D.plot_3D_scatter(
                vec_x,
                vec_y,
                vec_z,
                plot_scatter,
    ...
    def plot_1d(self):
        ...
        if (x_axis_name in LIST_AXIS_TO_PLOT["fit"].keys()) or (y_axis_name in LIST_AXIS_TO_PLOT["fit"].keys()):
            is_plot_with_error = True

        # is_plot_with_error = False  # REMOVE that line once the error bars are correctAT
        if is_plot_with_error:
            self.parent.ui.graphicsView_fitResult.plot_scatter_with_errors(
                vec_x=axis_x_data,
                vec_y=axis_y_data,
            ...
            self.parent.ui.graphicsView_fitResult.plot_scatter(axis_x_data, axis_y_data, x_axis_name, y_axis_name)
    ...
    def get_function_parameter_data(self, param_name):
        ...
        param_names, param_data = self.parent._core.get_peak_fitting_result(
            self.parent._project_name, 0, return_format=dict, effective_parameter=False
        )
