...
class ExportCSV(Export):
    def select_output_folder(self):
        out_folder = browse_dir(
            self.parent, caption="Choose where to create the CSV file", default_dir=self.parent._core.working_dir
        )
    ...
    def create_csv(self):
        try:
            ...
            generator = SummaryGenerator(self._csv_file_name, log_list=sample_logs.keys())

            generator.setHeaderInformation(
                {
                    "cal_wavelength": self.parent.hidra_workspace.get_wavelength(False, False),
            ...
            new_message = self.parent.current_root_statusbar_message + "\t\t\t\t Last Exported CSV: {}".format(
                self._csv_file_name
            )
