...
fitting_array = np.arange(NUM_PIXEL_1D - 40 - 40)
...
class PeakFitCalibration:
    ...
    def __init__(self, hb2b_inst=None, powder_engine=None, pin_engine=None, powder_lines=None,
                 single_material=True, out_of_plane_chunks=8):
        """
        Initialization
        ...
        self.chunks = out_of_plane_chunks
        self.vertical_slice = int((1024 - 40 - 40) / self.chunks)
    ...
    def FitDetector(self, fun, x0, jac='3-point', bounds=[], method='trf', ftol=1e-08, xtol=1e-08, gtol=1e-08,
                    ...
                    factor=100.0, i_index=2, fDiff=1e-9, start=0, stop=0):
    ...
    def get_alignment_residual(self, x, ConPeaks=False, start=0, stop=0):
        ...
        for engine_setup in self.engines:
            ...
            for i_tth in sub_runs:
                ...
                pyrs_reducer.build_instrument_prototype(-1. * self._engine.get_sample_log_value('2theta', i_tth),
                                                        ...
                                                        self._calib[0], self._calib[1], self._calib[2],
                                                        self._calib[3], self._calib[4], self._calib[5])

                tths = pyrs_reducer._instrument.get_pixels_2theta(1)

                tth_matrix = tths.reshape((NUM_PIXEL_1D, NUM_PIXEL_1D))[40:NUM_PIXEL_1D - 40, 40:NUM_PIXEL_1D - 40]
                detector_counts = self._engine.get_detector_counts(i_tth).reshape((NUM_PIXEL_1D, NUM_PIXEL_1D))
                detector_counts = detector_counts[40:NUM_PIXEL_1D - 40, 40:NUM_PIXEL_1D - 40]
                ...
                # define peak setup
                for ipeak in range(CalibPeaks.shape[0]):
                    Peaks = []
                    pars1 = {}
                ...
                if CalibPeaks.shape[0] >= 1:
                    for i_roi in range(self.vertical_slice):

                        # fit peaks
                        i1 = i_roi * self.chunks
                        i2 = (i_roi+1) * self.chunks
                        Fitresult = self.FitPeaks(tth_matrix[:, i1:i2].sum(axis=1) / self.chunks,
                                                  detector_counts[:, i1:i2].sum(axis=1), pars1, Peaks)
