...
def test_fuse_with(fuse_data):
    ...
    for i in range(1, 1 + fuse_data["logs_count"]):
        ...
        scalar_field_samples.append(
            ScalarFieldSample(fuse_data["name"], logs["values"], logs["errors"], logs["x"], logs["y"], logs["z"])
        )

    # Fuse the scalar field sample objects
    fused_field = fuse_scalar_field_samples(
        *scalar_field_samples, criterion=fuse_data["criterion"], resolution=fuse_data["resolution"]
    )
...
def data_interpolate_surface_scans():
    ...
    return {
        ...
        "name": "strain",
        "sample1": _lf(
            [0, 0, 0, 0, 0, 3, 3, 3, 3, 3, 6, 6, 6, 6, 6], [0, 4, 8, 12, 16, 0, 4, 8, 12, 16, 0, 4, 8, 12, 16]
        ),
        "sample2": _lf(
            [
                10,
...
def test_interpolate_surface_scans(data_interpolate_surface_scans, allclose_with_sorting):
    ...
    fused = fuse_scalar_field_samples(
        *scalar_field_samples, criterion=data["criterion"], resolution=data["resolution"]
    )
    ...
    interpolated = fused.interpolated_sample(keep_nan=True, criterion=data["criterion"], resolution=data["resolution"])
...
def test_create_stress_1(data_create_stress_1):
    ...
    #
    # Diagonal stress
    poisson_ratio = 1.0 / 3.0  # makes nu / (1 - 2*nu) == 1
    ...
    assert np.allclose(
        stress.strain33.values, -(stress.strain11.values + stress.strain22.values), equal_nan=True, atol=1
    )
