...
def test_default_calibration_file():
    ...
    """
    default_calib_file = os.path.join(ReductionController.get_default_calibration_dir(), "HB2B_Latest.json")
...
@pytest.mark.parametrize(
    ...
    [
        ("/HFIR/HB2B/IPTS-22731/nexus/HB2B_1017.nxs.h5", None, None, "tests/data/HB2B_1017_NoMask_Gold.h5"),
        (
            "/HFIR/HB2B/IPTS-22731/nexus/HB2B_1017.nxs.h5",
            None,
            "tests/data/HB2B_Mask_12-18-19.xml",
...
def test_manual_reduction(nexus_file, calibration_file, mask_file, gold_file):
    ...
    other_file = "Gold_{}_Cal{}.h5".format(mask_file is not None, calibration_file is not None)
...
def test_load_split():
    ...
    controller.reduce_hidra_workflow(
        nexus_file, output_dir, progressbar=None, calibration=None, mask=None, project_file_name=target_file_path
    )
    ...
    assert 78 < vec_2theta.mean() < 82, "2theta range ({}, {}) shall be centered around 100 for sub run 2.".format(
        vec_2theta[0], vec_2theta[-1]
    )

    assert vec_intensity[~np.isnan(vec_intensity)].max() > 2, "Max intensity {} must larger than 2".format(
        vec_intensity[~np.isnan(vec_intensity)].max()
    )

    # Sample logs
    assert abs(controller.get_sample_log_value("2theta", 1) - 69.99525) < 1e-5
    assert abs(controller.get_sample_log_value("2theta", 2) - 80.0) < 1e-5
