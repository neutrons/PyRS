...
class PeakFittingTest:
    ...
    def __init__(self, input_file_name):
        ...
        hd_ws = self._reduction_controller.load_hidra_project(
            input_file_name, project_name=self._project_name, load_detector_counts=False, load_diffraction=True
        )
    ...
    def show_fit_result(self, show_effective_params):
        ...
        _, peak_params_matrix = self._reduction_controller.get_peak_fitting_result(
            self._project_name, return_format=numpy.ndarray, effective_parameter=show_effective_params
        )
        # Plot peak width
        ...
        peak_params_dict = self._reduction_controller.get_peak_fitting_result(
            self._project_name, return_format=dict, effective_parameter=True
        )
...
@pytest.mark.parametrize(
    "source_project_file, output_project_file, peak_type, peak_info_list",
    [
        (
            "data/HB2B_1017.h5",
            "HB2B_1017_2Peaks.h5",
            "PseudoVoigt",
            [PeakInfo(81.0, 78.0, 83.0, "LeftPeak"), PeakInfo(85.0, 83.0, 87.0, "RightPeak")],
        ),
        (
            "/HFIR/HB2B/IPTS-22731/shared/ProjectFiles/HB2B_1065.h5",
            "HB2B_1065_Peak.h5",
            "PseudoVoigt",
            [PeakInfo(90.5, 89.9, 91.6, "311")],
        ),
    ],
...
@pytest.mark.parametrize(
    "source_project_file, output_project_file, peak_type, peak_info",
    [
        (
            "/HFIR/HB2B/IPTS-22731/shared/ProjectFiles/HB2B_1065.h5",
            "HB2B_1065_Peak.h5",
...
def test_retrieve_fit_metadata(source_project_file, output_project_file, peak_type, peak_info):
    ...
    hd_ws = controller.load_hidra_project(
        source_project_file, project_name=project_name, load_detector_counts=False, load_diffraction=True
    )
    ...
    # Fit peak
    fit_result = fit_engine.fit_peaks(peak_tag=peak_info.tag, x_min=peak_info.left_bound, x_max=peak_info.right_bound)
...
@pytest.mark.parametrize(
    "project_file_name, peak_file_name, peak_type, peak_info",
    [
        (
            "data/Hidra_16-1_cor_log.h5",
            "Hidra_16-1_cor_log_peak.h5",
            "Gaussian",
            PeakInfo(94.5, 91, 97, "Fe111"),
        ),  # NSFR2 peak
        ("data/HB2B_938.h5", "HB2B_938_peak.h5", "PseudoVoigt", PeakInfo(95.5, 91, 97, "Si111")),
    ],
...
def test_improve_quality():
    ...
    # create a controller from factory
    fit_engine = PeakFitEngineFactory.getInstance(
        hd_ws, peak_function_name=peak_type, background_function_name="Linear", wavelength=wavelength
    )

    # Fit peak @ left
    peak_info_left = PeakInfo(91.7, 87.0, 93.0, "Left Peak")

    fit_result = fit_engine.fit_peaks(
        peak_tag=peak_info_left.tag, x_min=peak_info_left.left_bound, x_max=peak_info_left.right_bound
    )
    assert fit_result
...
def test_write_csv():
    ...
    peaks.set_peak_fitting_values(subruns, data, error, [10.0, 20.0, 30.0])

    # create a SampleLog
    sample = SampleLogs()
    sample.subruns = subruns
    sample["variable1"] = numpy.linspace(0.0, 100.0, len(subruns))
    sample["variable2"] = numpy.linspace(100.0, 200.0, len(subruns))  # not to be found in the output
    sample["constant1"] = numpy.linspace(1.0, 1.0 + 5e-11, len(subruns))  # values less than tolerance
    sample["string1"] = numpy.array(["a constant string"] * sample.subruns.size)  # will be constant as well

    # write things out to disk
    generator = SummaryGenerator(csv_filename, log_list=["variable1", "constant1", "missing1", "string1"])
...
@pytest.mark.parametrize(
    "project_file_name, csv_filename, expected_header, num_subruns, num_logs, startswith, endswith",
    [
        (
            "/HFIR/HB2B/shared/PyRS/HB2B_1065_Peak.h5",
            "HB2B_1065.csv",
            EXPECTED_HEADER_1065,
            99,
            6,
            "sub-run,vx,vy,vz,",
            ",311_chisq",
        ...
        (
            ...
            1,
            3,
            "sub-run,vx,vy,vz,Si111_dspacing_center",
            ",Si111_chisq",
        ),
    ],
    ids=["HB2B_1065_CSV", "HB2B_938_CSV"],
)
def test_write_csv_from_project(
    project_file_name, csv_filename, expected_header, num_subruns, num_logs, startswith, endswith
):
