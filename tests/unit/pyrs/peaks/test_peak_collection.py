...
def test_peak_collection_init():
    ...
    assert PeakCollection("test2", "Gaussian", "Linear", wavelength=22.0)
...
def check_peak_collection(
    ...
    target_d_spacing_center_error=np.asarray([0.0, 0.0]),
    target_strain=np.nan,
    target_strain_error=np.asarray([0.0, 0.0]),
):
    ...
    if peak_shape == "PseudoVoigt":
        ...
        np.testing.assert_allclose(obs_eff_peaks["FWHM"], 2.0 * np.sqrt(2.0 * np.log(2.0)) * raw_peaks_array["Sigma"])

    np.testing.assert_equal(obs_eff_peaks["A0"], NUM_SUBRUN * [0.0])
    np.testing.assert_equal(obs_eff_peaks["A1"], NUM_SUBRUN * [0.0])
...
def test_peak_collection_Gaussian():
    ...
    check_peak_collection(
        ...
        wavelength=1.53229,
        d_reference=1.08,
        target_d_spacing_center=[1.08, 1.07],
        target_d_spacing_center_error=[0.0, 0.0],
        target_strain=[3234.0, -5408.0],
...
def test_peak_collection_lite():
    ...
    np.testing.assert_equal(obs, 0.0)
    np.testing.assert_equal(obs_error, 0.0)
