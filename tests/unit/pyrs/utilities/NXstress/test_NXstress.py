...
from pyrs.utilities.NXstress._fit import _Diffractogram, _Fit, _PeakParameters, _BackgroundParameters
...
class TestNXstress:
    ...
    def test_NXentry_multiple(
        ...
        with NXstress(file_path, "w") as nx:
            ...
            assert len(root.NXentry) == 2
            assert "entry" in root
            assert "entry_2" in root
    ...
    def test__Fit_fields_and_subgroups(
        ...
        required_fields = ("date", "program")
        required_subgroups = (
            (GROUP_NAME.DESCRIPTION, NXnote),
            (GROUP_NAME.INPUT, NXparameters),
    ...
    def test__PeakParameters_fields_and_subgroups(
        ...
        assert pytest.approx(peak_parameters["form_factor"], 1.0e-6) == (1.0 - peak0_params_value["Mixing"]).astype(
    ...
    def test__Diffractogram_fields_and_subgroups(
        ...
        required_attributes = ("signal", "auxiliary_signals", "axes")
        required_fields = (
            "scan_point",
