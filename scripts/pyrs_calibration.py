...
def main(argv=None):
    ...
    calibrator = mantid_peakfit_calibration.FitCalibration(
        nexus_file=nexus_file, eta_slice=ETA_Slice, bins=TTH_Bins, mask_file=DATA_MASK, vanadium=VAN_RUN
    )
    ...
    if CalibName is not None:
        ...
        if HFIR_CYCLE is not None:
            ...
            CalibName = "{}/cycle{}/HB2B_{}_{}.json".format(
                calib_folder_base, HFIR_CYCLE, calibrator.monosetting, datatime
            )
            calibrator.write_calibration(CalibName)
