...
def _parse_powder_line(json_entry):
    ...
    for item in json_entry:
        if isinstance(item, str) or isinstance(item, type("a")):
            hkl = item.split(" ")[1]
            h_ = float(hkl[0])
            k_ = float(hkl[1])
            l_ = float(hkl[2])
        ...
        if Input_error:
            raise RuntimeError(
                "{} is not in material list {}".format(item.split(" ")[0].lower(), list(_Materials.keys()))
            )
...
def main(argv=None):
    ...
    if calibration_inputs["POWDER_RUN"] is not None:
        calibration_inputs["POWDER_RUN"] = _load_nexus_data(
            calibration_inputs["ipts"], calibration_inputs["POWDER_RUN"], calibration_inputs["DATA_MASK"]
        )
        single_material = False
        mono = _get_mono_setting(calibration_inputs["POWDER_RUN"])

    if calibration_inputs["PIN_RUN"] is not None:
        calibration_inputs["PIN_RUN"] = _load_nexus_data(
            calibration_inputs["ipts"], calibration_inputs["PIN_RUN"], calibration_inputs["DATA_MASK"]
        )
        single_material = True
        mono = _get_mono_setting(calibration_inputs["PIN_RUN"])

    calibrator = peakfit_calibration.PeakFitCalibration(
        powder_engine=calibration_inputs["POWDER_RUN"],
